...
st.title("📊 Interactieve plot: de binomiale en Poissonverdeling")
with st.sidebar:
...
def draw_sample_poisson(lmbda):
    x = np.arange(0, max(2 * lmbda, 20))  # Mogelijke uitkomsten
    y = poisson.pmf(x, lmbda)  # Poisson-kansfunctie
    return x, y
